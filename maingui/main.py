...
class NMR_GUI(tk.Tk, Tecmag):
    ...
    def __init__(self, tecmag, tmag):
        ...
        self.slopar = {'Dwell Time':'DW', 'tau':'TAU', 'Acq. Points':'TD', 'Scans 1D':'NS', 'Actual Scans 1D':'NSC', 'Observe Freq.':'FR', 'd1':'D1','d2':'D2','d3':'D3','d4':'D4','d5':'D5','d6':'D6','d7':'D7','d8':'D8','d9':'D9',}
        self.sloadd = {'pulse':'PPFLIE', 'Date':'DATESTA', 'Exp. Start Time':'TIMESTA', 'Exp. Finish Time':'TTIMEEND', 'a1':'a1','a123':'a123', 'a2':'a2', 'a3':'a3', 'atn1':'atn1', 'atn2':'atn2', 'Receiver Gain':'RecGain', 'Receiver Phase':'RecPh', 'Filter':'Filter', 'Pcryo':'Pcryo', 'ad':'ad'}
...
# Tab Control
        ...
        tabControl.add(self.tab1, text='Parameters')
        tabControl.add(self.tab2, text='Additional')
        tabControl.add(self.tab6, text='Variables')
...
#             i += 1
        ...
        self.superdict = {}
        self.superdict.update(self.tecmag.dict_Acquisition)
        self.superdict.update(self.tecmag.dict_Frequency)
        self.superdict.update(self.tecmag.dict_Misc)
        self.superdict.update(self.tecmag.dict_Sequence)
...
# Tab 1 - Parameters
        ...
        for key, value in self.slopar.items():
            tk.Label(self.inner_frame1, text=value).grid(row=i % 5, column=2*(i//5), padx=5, pady=0, sticky="w")
            self.diks[key] = tk.Entry(self.inner_frame1)
            try:
                self.diks[key].insert(0, self.superdict[key])
            except:
                self.diks[key].insert(0, "N/A")
            self.diks[key].grid(row=i % 5, column=(2*(i//5) + 1), padx=5, pady=0, sticky="ew")
            self.diks[key].bind("<Return>", self.Focus)
            i += 1
...
# Tab 2 - Additional
        ...
        for key, value in self.sloadd.items():
            tk.Label(self.inner_frame2, text=value).grid(row=i % 5, column=2*(i//5), padx=5, pady=0, sticky="w")
...
# Tab 10 - Fourier
        ...
        self.size.insert(0, self.tecmag.dict_Acquisition['Acq. Points'])
        ...
        self.upper_bound.insert(0, self.tecmag.dict_Acquisition['Acq. Points'])
...
# Called functions
    ...
    def Focus(self, event):
        ...
        # self.superdict.update(self.tecmag.dict_Misc)
        # self.superdict.update(self.tecmag.dict_Sequence)
        # print(self.superdict)
        # for key, value in invers(self.superdict).items():
        #     self.superdict[value] = self.diks[key].get()
        # for key, value in self.tecmag.dict_Acquisition.items():
        #     try:
        #         self.tecmag.dict_Acquisition[key] = self.superdict[key]
        #     except:
        #         pass
        # for key, value in self.tecmag.dict_Frequency.items():
        #     try:
        #         self.tecmag.dict_Frequency[key] = self.superdict[key]
        #     except:
        #         pass
        # for key, value in self.tecmag.dict_Misc.items():
        #     try:
        #         self.tecmag.dict_Misc[key] = self.superdict[key]
        #     except:
        #         pass
        # for key, value in self.tecmag.dict_Sequence.items():
        #     try:
        #         self.tecmag.dict_Sequence[key] = self.superdict[key]
        #     except:
        #         pass
        # print(self.superdict)
...
# File handling
    def save_as_tnt(self):
        ...
        file_saveas = filedialog.asksaveasfilename(filetypes=file_type)
    ...
    def open_tnt(self):
        ...
        self.tecmag.dict_Sequence = {}
        file_type = [('TNT Files', '*.tnt')]
        tnt_path = filedialog.askopenfilename(filetypes=file_type)
    ...
    def Write(self, directory, name):  # zapise razultate                                              treba dodat da napise vse parametre ne sam tiste ki se setajo npr. pri avtomatizaciji
        # a = os.getcwd()
        a = directory
        a = a.replace('\\', '\\\\')
        params = {}
        ...
        with open(a + "\\" + name + '.dat', 'w', encoding='utf-8') as write_doc:
...
# Message
    ...
    def Update_GUI(self, updated_Acquisition, updated_Frequency, updated_Misc, updated_Sequence):
        superdicti = {}
        superdicti.update(updated_Acquisition)
        superdicti.update(updated_Frequency)
        superdicti.update(updated_Misc)
        superdicti.update(updated_Sequence)
        self.diks = {}
        print(superdicti)
        ...
        for key, value in self.slopar.items():
            ...
            self.diks[key] = tk.Entry(self.inner_frame1)
            try:
                self.diks[key].insert(0, superdicti[key])
            except:
                self.diks[key].insert(0, "N/A")
            self.diks[key].grid(row=i % 5, column=(2*(i//5) + 1), padx=5, pady=0, sticky="ew")
            self.diks[key].bind("<Return>", self.Focus)
